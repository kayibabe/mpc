...
export default function ZamzamClaimForm() {
  const [form, setForm] = useState({
    employeeName: "", postalAddress: "", phone: "", email: "",
    patientName: "", relationship: "", dob: "", age: "", signature: "",
    doctorNo: "", invoiceNo: "", dateClaimClosed: "", refDocNo: "",
    treatments: [{ lineNo: "", tariffCode: "", description: "", qty: "", yr: "", mth: "", days: "", feeCharged: "" }],
    diagnosis: "", isClosed: false,
  });

  const addTreatment = () => {
    setForm({...form, treatments: [...form.treatments, { lineNo: "", tariffCode: "", description: "", qty: "", yr: "", mth: "", days: "", feeCharged: "" }]});
  };
  ...
  return (
    <form className="space-y-6 bg-card border border-border/60 rounded-xl p-6">
      <div className="space-y-4">
        <h3 className="font-heading font-semibold text-sm uppercase tracking-wide">Employee Information</h3>
        ...
        <div className="space-y-2 text-xs">
          {form.treatments.map((t, idx) => (
            <div key={idx} className="grid grid-cols-12 gap-2 p-2 border border-border/60 rounded-lg">
              <input className="col-span-1 rounded border border-border px-2 py-1" placeholder="Line" value={t.lineNo} onChange={e => handleTreatmentChange(idx, "lineNo", e.target.value)} />
              <input className="col-span-1 rounded border border-border px-2 py-1" placeholder="Code" value={t.tariffCode} onChange={e => handleTreatmentChange(idx, "tariffCode", e.target.value)} />
              ...
              <input className="col-span-2 rounded border border-border px-2 py-1" placeholder="Fee" type="number" value={t.feeCharged} onChange={e => handleTreatmentChange(idx, "feeCharged", e.target.value)} />
      ...
      <button type="submit" className="w-full bg-primary text-primary-foreground rounded-lg py-2.5 font-medium text-sm hover:bg-primary/90 flex items-center justify-center gap-2"><Save className="w-4 h-4" /> Save Zamzam Claim</button>
    </form>
  );
}